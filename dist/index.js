...
async function run() {
    ...
    const cmApiToken = (0, core_1.getInput)("CLOUD_MAKER_TOKEN");
    const cmPipelineId = (0, core_1.getInput)("CLOUD_MAKER_PIPELINE_ID");
    const cmStageId = (0, core_1.getInput)("CLOUD_MAKER_STAGE_ID");
    ...
    if (cmPipelineId && !(0, uuid_1.validate)(cmPipelineId)) {
        console.warn("Input 'CLOUD_MAKER_PIPELINE_ID' is not a valid UUID");
        inputsAreValid = false;
    }
    if (cmStageId && !(0, uuid_1.validate)(cmStageId)) {
    ...
    const createResult = await (0, axios_1.default)({
    ...
    let lastSeenLog = 0;
    while (true) {
        const statusResult = await (0, axios_1.default)({
            url: `${cmBaseAddress}/deployments/${id}`,
            method: "GET",
            headers,
        });
        const { status, logs } = statusResult.data;
        ...
        if (status === "Succeeded") {
            ...
            (0, core_1.setFailed)("Deployment failed");
            break;
        }
...
run().catch((error) => {
    if (axios_1.default.isAxiosError(error)) {
        ...
        (0, core_1.setFailed)(`Deployment failed. (${status}) ${message}${message.endsWith(".") ? "" : "."} Please ensure that all inputs are set correctly.`);
    }
    else {
